...
\begin{enumerate}
   \item Software in bedeutenen Systemen (Flughafen, Flugzeug, Rakete) enthielt
      Fehler, was zu teils teuren und teil gefährlichen Ausfällen führte. %TODO: elaborate
   \item %TODO: NATO Consortium, development of development processes and practices akin to engineering disciplines
...
\begin{tikzpicture}[every node/.style={font=\footnotesize\sffamily}]
   ...
   \node () at (subFields-3) [fill=blue!20,text opacity=1] {$\triangleleft$ consists of};
   ...
   \node () at (makeMove-1) [fill=blue!20,text opacity=1] {$\triangledown$ make move}; 
\end{umlpackage}
\end{tikzpicture}
\end{document}
